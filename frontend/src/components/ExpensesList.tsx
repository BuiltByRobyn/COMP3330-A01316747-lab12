...
export function ExpensesList() {
  ...
  if (isError) {
    return (
      <div className="rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700 mt-4">
        <p>Could not load expenses. Please try again.</p>
        <button
          className="mt-2 rounded border border-red-300 bg-white px-3 py-1 text-xs text-red-700 hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={() => refetch()}
          disabled={isFetching}
        >
  ...
  if (!data?.expenses || data.expenses.length === 0) {
    return (
      <div className="rounded border bg-white p-6 text-center mt-4">
